...
public class WindowUtils {
	...
	public static void setSystemLAF() {
		try {
			...
			Logger.getLogger(WindowUtils.class.getName()).warning(
					"Unable to load Nimbus LaF");
			try {
				UIManager.setLookAndFeel(UIManager
						.getSystemLookAndFeelClassName());
			} catch (Exception e1) {
				Logger.getLogger(WindowUtils.class.getName()).severe(
						e1.getMessage());
			}
	...
	public static void errorDialog(Component parent, String msg, String title) {
		JOptionPane.showMessageDialog(parent, msg, (title == null ? "Error"
				: title), JOptionPane.ERROR_MESSAGE);
	}

	public static void warnDialog(Component parent, String msg, String title) {
		JOptionPane.showMessageDialog(parent, msg, (title == null ? "Warning"
	...
	public static void dumpComponentsBounds(Component c, int level) {
		...
		if (c instanceof Container) {
			...
			for (int j = 0; j < children.length; j++)
				dumpComponentsBounds(children[j], level + 1);
		}
	}

	public static void maximizeJFrameOnScreen(JFrame frame) {
